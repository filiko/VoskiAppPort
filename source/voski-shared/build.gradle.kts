...
/**
 * Generates a `BuildVersions` file in voski-shared/build/generated-src containing the current git commit and the lightning-kmp version.
...
val buildVersionsTask by tasks.registering(Sync::class) {
    from(
        resources.text.fromString(
            """
            |package com.voski.wallet.shared
            |
            |object BuildVersions {
            |    const val VOSKI_COMMIT = "${gitCommitHash()}"
        ...
        it.binaries {
            ...
            configureEach {
                it.compilations.all {
                    ...
                    // workaround for IndexOutOfBoundsException with voski-shared:linkReleaseFrameworkIosArm64 in iOS Archive build
...
sqldelight {
    databases {
        create("ChannelsDatabase") {
            packageName.set("com.voski.wallet.db.sqldelight")
...
if (includeAndroid) {
    extensions.configure<com.android.build.gradle.LibraryExtension>("android") {
        namespace = "com.voski.wallet.shared"
        compileSdk = 34
        defaultConfig {
            minSdk = 26
